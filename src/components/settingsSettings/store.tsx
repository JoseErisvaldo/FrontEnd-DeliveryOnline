...
export default function Store() {
  ...
  if (error) return <Alerts message={{ title: "Erro ao buscar os estabelecimentos", description: error }} type="destructive" />;
  if (establishments.length === 0) return <Alerts message={{ title: "Nenhum estabelecimento encontrado", description: "Por favor, adicione um estabelecimento para continuar." }} />;
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
      {establishments.map((est) => (
        <Card key={est.id} className="hover:shadow-lg transition-shadow duration-200">
          <CardHeader>
            <div className="flex flex-col">
              <h2 className="font-bold text-lg flex gap-2 items-center"><House className="w-4 h-4" />{est.name}</h2>
